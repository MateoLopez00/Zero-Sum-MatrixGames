    ...
    for adv_type in ADVERSARIES:
        ...
        for name in ALGORITHM_ORDER:
            ...
            if name == "UCB":
                ...
                records = _run_our_algorithm_convergence(A_GAME, horizon, n_runs, adv_type, sigma, rng, checkpoints)
            else:
...
def run_convergence_and_plot(
...
) -> tuple[list[tuple[dict[int, dict[str, dict[str, list[float]]]], dict[str, Any]]], list[Path]]:
    ...
    here = Path(__file__).resolve().parent
    plots_dir = here / "plots"
    payloads: list[tuple[dict[int, dict[str, dict[str, list[float]]]], dict[str, Any]]] = []
    paths: list[Path] = []
