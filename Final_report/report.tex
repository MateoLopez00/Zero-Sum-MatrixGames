...
The results should not be read as a contradiction of the paper when they differ from the baseline: the opponent class has changed. In the Section 3 plot, \OurAlg{}--\OurAlg{} has the largest cumulative Nash-regret growth in this run, while Nash--Nash stays nearly flat because both sides remain close to the equilibrium-like fixed strategy. Mixed pairs such as \OurAlg{}--Nash and \OurAlg{}--Hedge sit between these extremes, showing how the row learner's regret changes when the column side also follows a learning rule. In Section 4, UCB--UCB grows the most, EXP3--EXP3 is also high, and \OurAlg{}--\OurAlg{} plateaus below those two symmetric generic-bandit pairings. The main conclusion is that bilateral learning exposes a different regime of the same games: it is relevant for understanding adaptive multi-agent play, but it is not the setting covered by the original regret theorem.
